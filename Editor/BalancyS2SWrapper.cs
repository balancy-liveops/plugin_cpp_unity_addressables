...
namespace Balancy.Editor
{
    public class BalancyS2SWrapper
    {
        ...
        public AddressablesEditorUtils.ServerRequest CreateRequest(string endpoint, string method = "GET", string branchName = null)
        {
            // Form the full URL, replacing placeholders
            string fullUrl = BASE_URL + endpoint;

            Debug.LogWarning(" fullUrl = " + fullUrl);
            bool isPost = method == "POST" || method == "PUT" || method == "PATCH";
            var request = new AddressablesEditorUtils.ServerRequest(fullUrl, isPost)
            {
                Method = method
            };
        ...
        public AddressablesEditorUtils.ServerRequest SignRequest(AddressablesEditorUtils.ServerRequest request)
        {
            // Generate timestamp (Unix timestamp in milliseconds)
            long timestamp = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();

            // Generate nonce (32 characters) using C++ method
            string nonce = GenerateNonce(32);

            // Form JSON data for signature
            string jsonData = GetJsonDataForSignature(request);

            // Calculate signature using C++ method
        ...
        public void SendRequest(AddressablesEditorUtils.ServerRequest request, Action<UnityWebRequest> callback)
        {
            var helper = EditorCoroutineHelper.Create();
            var signedRequest = SignRequest(request);

            helper.LaunchCoroutine(AddressablesUnityUtils.SendRequest(signedRequest, callback));
        }
        ...
        /// </summary>
        private string GetJsonDataForSignature(AddressablesEditorUtils.ServerRequest request)
        {
        ...
        /// </summary>
        public AddressablesEditorUtils.ServerRequest AddOrUpdateTemplateEntities(string branchName, string templateName, string primaryKey, List<Dictionary<string, object>> entities, Dictionary<string, string> primaryKeys = null)
        {
        ...
        /// </summary>
        public AddressablesEditorUtils.ServerRequest DeleteTemplateEntities(string branchName, string templateName, string primaryKey, List<string> values)
        {
        ...
        /// </summary>
        public AddressablesEditorUtils.ServerRequest GetTemplateEntityByPrimaryKey(string branchName, string templateName, string primaryKey, string primaryValue, Dictionary<string, string> primaryKeys = null)
        {
